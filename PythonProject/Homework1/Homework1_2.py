...
while True:
    ...
    TemperatureStr = input("请输入带币种符号的金额>:")
    TemperatureSign = TemperatureStr[-1]                            # 获取用户输入的金额字符串的最后一位，即币种符号

    while TemperatureSign not in ['¥', '￥', '$', '＄', '€']:              # 若用户输入有误，则循环直至输入正确为止
        TemperatureStr = input("您的输入有误!请重新输入带币种符号的金额>:")      # 2023.03.07程序编写最终位置
